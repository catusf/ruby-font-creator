...
segoeui_font_path = os.path.join('./output', 'segoeui.ttf')  # Segoe UI font in the output folder
...
def create_image_with_text(font_path, text, image_size, background_color, text_color, segoeui_font_path):
    ...
    try:
        segoeui_font = ImageFont.truetype(segoeui_font_path, size=30)  # Use Segoe UI for the font name
    except IOError:
        segoeui_font = ImageFont.load_default()  # Fallback to default if Segoe UI is not found
    
    # First, write the font name using Segoe UI
    font_name_bbox = draw.textbbox((0, 0), font_name, font=segoeui_font)  # Use textbbox instead of textsize
    font_name_width, font_name_height = font_name_bbox[2] - font_name_bbox[0], font_name_bbox[3] - font_name_bbox[1]
    font_name_position = ((image_size[0] - font_name_width) // 2, 10)  # Position the font name at the top center
    ...
    while True:
        ...
        text_bbox = draw.textbbox((0, 0), full_text, font=font)  # Use textbbox instead of textsize
...
def generate_images_from_ttf():
    ...
    ttf_files = [f for f in os.listdir(output_dir) if f.endswith('.ttf') and f != 'segoeui.ttf']
    
    if not ttf_files:
        print("No TTF font files found in the output directory (excluding segoeui.ttf).")
        return
    
    for ttf_file in ttf_files:
        font_path = os.path.join(output_dir, ttf_file)
        image = create_image_with_text(font_path, text, image_size, background_color, text_color, segoeui_font_path)
        image.save(os.path.join(output_dir, f'{ttf_file.replace(".ttf", "")}.png'))
    
    print(f"Images have been created and saved successfully for {len(ttf_files)} font(s).")
