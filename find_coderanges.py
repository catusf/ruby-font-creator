...
def get_unicode_ranges_with_names(font_path):
    ...
    for start, block_name, end in UNICODE_BLOCKS:
        block_covered = [code_point for code_point in code_points if start <= code_point <= end]
        
        if block_covered:
            block_ranges.append((block_name, start, end, len(block_covered), len(block_covered) / (end - start + 1) * 100))
    ...
    for block_name, start, end, covered, percentage in block_ranges:
        print(f"  {block_name}: U+{start:04X} - U+{end:04X}, Covered: {covered} ({percentage:.2f}%)")
    ...
    print(f"Covered code points: {covered_code_points} ({(covered_code_points / total_code_points) * 100:.4f}%)")
